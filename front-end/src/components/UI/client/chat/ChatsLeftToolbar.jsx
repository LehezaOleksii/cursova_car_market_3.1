...
const ChatsLeftToolbar = ({ chats, setChats, onSelectChat, setUnreadMessagesCount }) => {
  ...
  const fetchChatRooms = async () => {
    try {
      const response = await fetch(`http://auto-market-backend:8080/chat/rooms/userId/${id}`, {
        method: 'GET',
  ...
  const searchExternalChats = async (name) => {
    try {
      const response = await fetch(`http://auto-market-backend:8080/chat/users/${id}?name=${name}`, {
        method: 'GET',
        headers: { Authorization: `Bearer ${jwtStr}` },
      });
