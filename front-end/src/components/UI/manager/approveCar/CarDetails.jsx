...
const CarDetails = () => {
  ...
  const handleApproveCar = async () => {
    const url = `http://localhost:8080/vehicles/${carId}/approve`;
    ...
    if (response.ok) {
      navigate('/manager/approve/cars');
    } else {
      console.error("Error approving car:", response.statusText);
    }
  };

  const handleDisapproveCar = async () => {
    const url = `http://localhost:8080/vehicles/${carId}/disapprove`;
    const response = await fetch(url, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
    ...
    if (response.ok) {
      ...
      console.error("Error disapproving car:", response.statusText);
    }
  };
            ...
            <div className="card">
              <div className="card-body">
                ...
                <div className="d-flex justify-content-end mt-3">
                  <button className="btn btn-primary me-2" onClick={handleApproveCar}>Approve</button>
                  <button className="btn btn-danger" onClick={handleDisapproveCar}>Disapprove</button>
                </div>
              </div>
            </div>
