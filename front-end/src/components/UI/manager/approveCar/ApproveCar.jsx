...
import "../../client/DashboardCar/car_style.css";
...
const ApproveCar = ({ car, removeCarFromList }) => {
  ...
  const blockCar = async () => {
    const url = `http://auto-market-backend:8080/vehicles/${car.id}/disapprove`;
    await fetch(url, {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
  ...
  const approveCar = async () => {
    const url = `http://auto-market-backend:8080/vehicles/${car.id}/approve`;
    await fetch(url, {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
  ...
  const handleDetailsCar = (carId) => {
    navigate(`/car/${carId}`);
  };

  return (
    <div className="card mb-4 box-shadow-12 br24">
          ...
          <div className="card-body">
            <div className="row">
              ...
              <div className="col-5 text-end">
                <span className={`badge rounded-pill   ${getStatusBadgeColor()}`}>{`Status: ${car.status.replaceAll("_", " ").charAt(0).toUpperCase() + car.status.replaceAll("_", " ").slice(1).toLowerCase()}`}</span>
              </div>
            </div>
            <div>
            ...
            <div className="mt-3 d-flex align-items-center gap-2 flex-wrap mt-4">
              ...
              <div className="d-flex align-items-center gap-2" style={{ marginLeft: "auto" }}>
                {car.status === "ON_MODERATION" ? (
                  <>
                    <button
                      onClick={() => handleDetailsCar(car.id)}
                      className="btn btn-primary br24 box-shadow-12"
                      style={{ padding: "7px", width: "85px" }}
                    >
                      Details
                    </button>
                    <button className="btn btn-primary br24 box-shadow-12" style={{ padding: "7px", width: "110px" }} onClick={approveCar}>
                      Approve Car
                    </button>
                  ...
                  <>
                    ...
                    <button onClick={handleNavigate} className="btn btn-primary br24 box-shadow-12" style={{ padding: "7px", width: "195px" }}>
                      Change Car Information
                    </button>
                    <button className="btn btn-danger br24 box-shadow-12" style={{ padding: "7px", width: "105px" }} onClick={() => setShowConfirm(true)}>
                      Remove Car
                    </button>
                  </>
                )}
          ...
          <div className="modal-dialog modal-dialog-centered">
            <div className="modal-content">
              ...
              <div className="modal-footer">
                <button
                  type="button"
                  className="btn btn-secondary br24"
                  onClick={() => setShowConfirm(false)}
                >
                ...
                <button
                  type="button"
                  className="btn btn-danger br24"
                  onClick={() => {
                    handleRemoveCarSubmit(car.id);
                    setShowConfirm(false);
                  }}
                >
