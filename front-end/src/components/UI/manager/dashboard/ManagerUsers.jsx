...
const ManagerUsers = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      let url = `http://auto-market-backend:8080/users?page=${currentPage}&size=${pageSize}`;
        if (filter !== baseFilter) {
        url = `http://auto-market-backend:8080/users/filter?page=${currentPage}&size=${pageSize}`;
      }
  ...
  useEffect(() => {
    const fetchUserRole = async () => {
      try {
        const url = `http://auto-market-backend:8080/users/role`;
        const response = await fetch(url, {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
